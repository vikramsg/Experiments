...
def run_synthetic_generation_db(
...
def run_experiment_and_monitor(
    ...
    args_str = (
        f"--model-id UsefulSensors/moonshine-tiny --dataset-path db://{train_dataset} "
        f"--manifest-path db://{eval_dataset} --safety-manifest-path db://{heldout_dataset} "
...
if __name__ == "__main__":
    ...
    synth_mini_id = run_synthetic_generation_db(
        num_samples=2, dataset_name="synth_mini_db", audio_prefix="synth_mini_db"
    )
