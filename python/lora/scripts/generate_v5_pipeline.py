...
def run_synthetic_generation(num_samples: int, manifest_path: str, audio_prefix: str):
    ...
    for i, target_text in enumerate(prompts):
        ...
        # NOTE: Save as 24kHz. Moonshine will resample it down to 16kHz during training internally anyway.
...
def run_experiment_and_monitor(experiment_name: str, train_manifest: str, max_steps: int, eval_interval: int):
    logger.info("Launching experiment %s...", experiment_name)
    
    cmd = [
        "just", "run-experiment", experiment_name,
        f'"--model-id UsefulSensors/moonshine-tiny --train-manifest {train_manifest} --eval-manifest data/my_voice_eval.jsonl --heldout-manifest data/heldout_manifest.jsonl --max-steps {max_steps} --eval-interval {eval_interval} --learning-rate 1e-4 --lora-r 32 --lora-alpha 64 --lora-dropout 0.1 --use-dora"'
    ]
...
if __name__ == "__main__":
    ...
    run_synthetic_generation(num_samples=5, manifest_path=mini_synth_manifest, audio_prefix="synth_mini")
    mix_datasets(synthetic_manifest=mini_synth_manifest, output_manifest=mini_mixed_manifest)
    run_experiment_and_monitor(experiment_name=mini_exp_name, train_manifest=mini_mixed_manifest, max_steps=50, eval_interval=25)
    ...
    run_synthetic_generation(num_samples=505, manifest_path=full_synth_manifest, audio_prefix="synth_full")
    mix_datasets(synthetic_manifest=full_synth_manifest, output_manifest=full_mixed_manifest)
    run_experiment_and_monitor(experiment_name=full_exp_name, train_manifest=full_mixed_manifest, max_steps=1000, eval_interval=100)
