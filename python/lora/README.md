...
- Normalization/decode parity: Using matching normalization and decode settings across training/eval/inference.
  - Enforced by configuration and code paths (for example, shared decode setup in `src/lora/model_utils.py` and inference path in `scripts/run_stt.py`); conceptual, not a standalone artifact.
