...
- Baseline: Metrics from the base model before LoRA training on a chosen manifest.
  - Generated during experiment runs in `src/lora_training/runners/experiment.py` (`baseline_wer`, `baseline_eval_loss`) and written to `experiment_metrics.json` under the run output directory.
- Tuned model: Base model with trained LoRA adapter applied.
  - Generated by training runs (`src/lora_training/runners/experiment.py`) as artifacts in:
...
- Normalization/decode parity: Using matching normalization and decode settings across training/eval/inference.
  - Enforced by configuration and code paths (for example, shared decode setup in `src/lora_training/model_utils.py` and inference path in `src/lora_training/transcribe.py`); conceptual, not a standalone artifact.
...
Unified experiment runs (`src/lora_training/runners/experiment.py`) write these outputs under `--output-dir`:
...
Small real-world runs (`src/lora_training/runners/real_small.py`) write:
- `real_metrics.json`
- `lora_adapter/`
- `processor/`
