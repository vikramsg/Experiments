...
{
   const char *mesh_file = "all_wall.msh";
   int    order      = 3;
   ...
   VectorFunctionCoefficient u_wall_bnd(aux_dim, wall_bnd_cnd); // Defines lid boundary condition
   // Linear form for boundary condition
...
//       cout << nodes(sub1) << '\t' << nodes(sub2) << '\t' << u_sol(sub4) << '\t' << rhs[sub1]<< endl;      
//       cout << nodes(sub1) << '\t' << nodes(sub2) << '\t' << u_sol(sub1) << '\t' << f_inv[5*offset + i]<< endl;      
//       cout << nodes(sub1) << '\t' << nodes(sub2) << '\t' << u_sol(sub2) << '\t' << aux_grad[3*offset + i]<< endl;      
//       cout << nodes(sub1) << '\t' << nodes(sub2) << '\t' << u_sol(sub1) << '\t' << b_aux_y_1[sub1]<< endl;      
//       cout << nodes(sub1) << '\t' << nodes(sub2) << '\t' << u_sol(sub1) << '\t' << b[sub3] << endl;      
...
void wall_bnd_cnd(const Vector &x, Vector &v)
{
   ...
   {
       double rho = 1;
       double p   = 2*mu*x(0) + 10;

       v(0) = x(1)*x(1);  // x velocity   
       v(1) = 0.0;  // y velocity 
       v(2) = p/(rho*R_gas);  // Temperature 
   }
}
