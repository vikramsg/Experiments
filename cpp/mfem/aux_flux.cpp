...
//      new DG_Inv_Characteristic_Integrator(
//      u_vec, u_char_bnd, var_dim, -1.0), dir_bdr); 
//   b1.Assemble();
//   ///////////////////////////////////////////////////////////
//   VectorFunctionCoefficient u_wall_bnd(dim, wall_bnd_cnd); // Defines wall boundary condition
//   // Linear form for boundary condition
//   dir_bdr     = 0; // Deactivate all boundaries
//
...
//      new DG_Inv_NoSlipWall_Integrator(
...
//   LinearForm b_aux(&fes);
//   b_aux.AddBdrFaceIntegrator(
//      new DG_Vis_Aux_NoSlipWall_Integrator(
//      aux_vec, u_wall_bnd, var_dim, 1.0, 0.5), dir_bdr);  // alpha = 1.0; beta = 0.0;
//   b_aux.Assemble();
//
//   FiniteElementSpace fes_aux_grad(mesh, &fec, dim*aux_dim);
//   GridFunction f_vis(&fes_vec);
//   GridFunction aux_grad(&fes_aux_grad);
//   getAuxGrad(dim, K_vis_x, K_vis_y, M, u_sol, aux_grad);
...
//
//   double thermal_k = (mu/Pr)*(gamm/(gamm - 1)); // k = mu*Cp/Pr
//   LinearForm b_vis(&fes);
//   b_vis.AddBdrFaceIntegrator(
//      new DG_Vis_NoSlipWall_Integrator(
//      f_vis_vec, aux_grad_vec, 1.0, 0.5, thermal_k), dir_bdr);  // alpha = 1.0; beta = 0.0;
//   b_vis.Assemble();
//   ///////////////////////////////////////////////////////////
...
//       cout << nodes(sub1) << '\t' << nodes(sub2) << '\t' << u_sol(sub4) << endl;      
//       cout << nodes(sub1) << '\t' << nodes(sub2) << '\t' << u_sol(sub4) << "\t" << u_sol(sub1)*(2.5*R_gas*aux_var(sub3) + 0.5*(pow(u_sol(sub2), 2) + pow(u_sol(sub3), 2))) << endl;      
//       cout << nodes(sub1) << '\t' << nodes(sub2) << '\t' << pow(aux_var(sub1), 2) + pow(aux_var(sub2), 2) << endl;      
...
//        cout << u(2*offset + j) << "\t" << aux_sol[offset + j] << endl;
//        cout << rho_E(j) << "\t" << u_sol(j) << endl;
    }
}
