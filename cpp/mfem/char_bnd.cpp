...
//   const char *mesh_file = "periodic-square.mesh";
   ...
   ///////////////////////////////////////////////////////////
   VectorFunctionCoefficient u_bnd(var_dim, char_bnd_cnd); // Defines characterstic boundary condition
   /////////////////////////////////////////////////////////////
   // Linear form for boundary condition
   Array<int> dir_bdr(mesh->bdr_attributes.Max());
   dir_bdr     = 0; // Deactivate all boundaries

   dir_bdr     = 1; // Activate all boundaries
...
void char_bnd_cnd(const Vector &x, Vector &v)
{
    ...
    double a = sqrt(gamm*p/rho); // Speed of sound
    double M = 0.5;              // Mach no
...
void init_function(const Vector &x, Vector &v)
{
   ...
   if (dim == 2)
   {
       double rho, u1, u2, p, E;
       double rho_0, p_0, E_0;

       rho_0 = 1;
       p_0   = 1;

       double a = sqrt(gamm*p_0/rho_0); // Speed of sound
       double M = 0.5;                  // Mach no
       ...
       E_0 = p_0/(gamm - 1) + 0.5*rho_0*v_sq;

       double    A   = 0.01, sigma = 0.01 ;

       double x_sq   = pow(x(0), 2) + pow(x(1), 2);

       double factor = (1 + A*exp(-x_sq/(2*pow(sigma, 2))));

       rho = rho_0*factor;
       p   = p_0;
       E   = (p_0/(gamm - 1))*factor + rho*v_sq;
