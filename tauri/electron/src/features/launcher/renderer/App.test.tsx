...
describe('Launcher App', () => {
  it('shows browser, notes, and opencode launch cards', () => {
    ...
    expect(screen.getByRole('heading', { level: 2, name: /terminal/i })).toBeVisible()
    expect(screen.getByRole('button', { name: /launch terminal/i })).toBeVisible()
  })
  ...
  it('opens the Terminal app when its launch button is clicked', async () => {
    const user = userEvent.setup()
    ...
    await user.click(screen.getByRole('button', { name: /launch terminal/i }))
